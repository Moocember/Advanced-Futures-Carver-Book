...
def calculate_stats(perc_return: pd.Series,
    ...
    DDs = DD(perc_return_at_freq.dropna())
    avg_drawdown = DDs.loc[DDs.values<0].mean()
    max_drawdown = maxDD(perc_return_at_freq.dropna())
...
def calculate_drawdown(perc_return):
    ...
    max_cum_perc_return = cum_perc_return.rolling(len(perc_return)+1,
                                                  min_periods=1).max()
    return cum_perc_return - max_cum_perc_return
...
if __name__ == '__main__':
    ...
    data = pd_readcsv('Data/sp500.csv')
    data = data.dropna()
    ...
    print(1)
    print(calculate_stats(perc_return, at_frequency=NATURAL))
    print(fullBacktest(perc_return.fillna(0)))
